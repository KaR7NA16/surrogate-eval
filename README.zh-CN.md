...
项目源于对部分观测混沌系统中扰动响应保真度的研究。冻结的 Lorenz–96 案例保留实验协议、模型权重、参考数据和结果，使通用软件接口与可追溯的研究流程相连接。

案例包含两项训练干预研究。每项研究均在固定选择后，对 **1,536 个新生成的初始状态**进行评估，覆盖 F16/F24，每个 forcing 下包含八个训练数据集组。软件可以重建所选模型的预测，并通过统一评分接口比较结果。

这项研究也展示了量化实际收益的必要性：完整网络干预对 history 输入模型的响应误差改善中位数为 **F16 的 3.03% 和 F24 的 0.88%**，低于原研究设定的实用阈值。这些结果构成可复现的评估案例，不构成训练算法优越性的证明。对照设置、统计分母和限制详见[完整证据记录](docs/evidence.md)。

历史证据包保存在仓库外，不随仓库或安装包分发，目前没有公开下载入口。[复现指南](reproduction/README.md)说明如何校验清单并导出数据；线性系统与非线性摆模型示例可独立运行。
...
引用元数据见 [CITATION.cff](CITATION.cff)，版本记录见 [CHANGELOG.md](CHANGELOG.md)。当前 0.1.0 为本地 Alpha 版本，尚未发布到公共软件包索引或分配 DOI。
