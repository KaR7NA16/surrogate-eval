...
|---|---|---|
| `surrogate_eval` | `PairedReference`, `Prediction`, `evaluate`, `compare`, `load_reference`, `load_prediction` | Versioned paired scoring |
...
`surrogate-eval --help` lists workflows; `surrogate-eval COMMAND --help` gives exact arguments. CLI errors handled by the interface return status 2 with a short message; success returns 0. `python -m surrogate_eval` is equivalent to `surrogate-eval`. Reports and NPZ schemas are version 1; this alpha release does not promise stable internal underscore-prefixed functions.
